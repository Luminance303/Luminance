...
CMD:ubound(playerid,params[])
{
	...
	if(AccountData[playerid][pAdmin] < 3) return SendClientMessage(playerid,COLOR_WHITE,"You need to be level 3 to use this command!");
	if(!strlen(tmp)) return SendClientMessage(playerid, COLOR_WHITE, "{6EF83C}Usage:{FFFFFF} /ubound [playerid]");
	if(!IsPlayerConnected(TargetID))return SendClientMessage(playerid,COLOR_WHITE,"Player is not connected!");
	new string[256];
	format(string, sizeof(string), "*[%s] %s has removed your World Boundaries *", AccountData[playerid][pAdminname],GetPlayerNameEx(TargetID));
	SendStaffMessage(X11_YELLOW, "*[%s] %s has removed your World Boundaries *", AccountData[playerid][pAdminname],GetPlayerNameEx(TargetID));
	SetPlayerWorldBounds(TargetID, 9999.9, -9999.9, 9999.9, -9999.9);
	if(TargetID != playerid) SendClientMessage(TargetID,COLOR_WHITE,string);
	else
	{
		format(string,sizeof(string),"* You have removed %s's World Boundaries *", GetPlayerNameEx(TargetID));
		SendClientMessage(TargetID,COLOR_WHITE,string);
		SendStaffMessage(X11_YELLOW, "* You have removed %s's World Boundaries *", GetPlayerNameEx(TargetID));
	}
	return 1;
}
CMD:setallinterior(playerid, params[])
{
	new tmp[256], Index, string[256], interior;
	tmp = strtok(params,Index);
	if(AccountData[playerid][pAdmin] < 4) return SendClientMessage(playerid,COLOR_WHITE,"You need to be level 4 to use this command!");
	if(!strlen(tmp)) return SendClientMessage(playerid, COLOR_WHITE, "{6EF83C}Usage:{FFFFFF} /setallinterior [int]");
	interior = strval(tmp);
	format(string, sizeof(string), "* Administrator %s has set all players in Interior %d *", interior);
...
CMD:fakecmd(playerid, params[])
{
	...
	if(AccountData[playerid][pAdmin] < 5) return SendClientMessage(playerid,COLOR_WHITE,"You need to be level 5 to use this command!");
	if(!strlen(tmp) ) return SendClientMessage(playerid, COLOR_WHITE, "{6EF83C}Usage:{FFFFFF} /fakecmd [playerid] [command]");
 	if(!IsPlayerConnected(TargetID))return SendClientMessage(playerid,COLOR_WHITE,"Player is not connected!");

	TargetID = strval(tmp);
	if(strfind(tmp2, "/", false) != -1)
	{
		CallRemoteFunction("OnPlayerCommandText", "is", TargetID, tmp2);
		SendClientMessage(playerid,COLOR_WHITE,"* Fake command sent! *");
	}
	else return SendClientMessage(playerid, COLOR_WHITE, "ERROR: Add '/' before putting the command name to avoid the command unknown error!");
	return 1;
}
...
CMD:getcar(playerid, params[])
{
	...
	if	(!AccountData[playerid][pAdminDuty])
		return Info(playerid, "Tidak bisa menggunakan perintah admin saat tidak bertugas");

	new vehicleid;
	if(sscanf(params, "d", vehicleid))
	    return Info(playerid, "/getcar [vehicle id]");
	...
	SendStaffMessage(COLOR_WHITE," %s telah menarik kendaraan dengan nomor ID %d", AccountData[playerid][pAdminname], vehicleid);
	return 1;
}
...
CMD:gotocar(playerid, params[])
{
	...
	if (sscanf(params, "d", vehicleid))
	    return Info(playerid, "/gotocar [vehicle id]");

	if (vehicleid < 1 || vehicleid > MAX_VEHICLES || !IsValidVehicle(vehicleid))
		return Info(playerid, "ID kendaraan yang kamu masukan tidak valid.");
...
{
	if(AccountData[playerid][pAdmin] < 3) return SendClientMessage(playerid,COLOR_WHITE,"You need to be level 3 to use this command!");
    if(!strlen(params)) return SendClientMessage(playerid, COLOR_WHITE, "{6EF83C}Usage:{FFFFFF} /setgravity [<-50.0 - 50.0->]");
	new string[128];
	new Float:Gravity = floatstr(params);
	format(string,sizeof(string),"* Admnistrator %s has set the Server Gravity to %f *",AccountData[playerid][pAdminname],Gravity);
	if(Gravity > 50 || Gravity < 50)
	{
		Gravity = 50.0;
	}
	SetGravity(Gravity);
...
CMD:ejectall(playerid,params[])
{
	...
	new string[128];
	format(string,sizeof(string),"* Administrator %s has Ejected all Players *", AccountData[playerid][pAdminname]);
	return SendClientMessageToAll(COLOR_WHITE, string);
}
CMD:uboundall(playerid,params[])
{
	new string[256];
	if(AccountData[playerid][pAdmin] < 4) return SendClientMessage(playerid,COLOR_WHITE,"You need to be level 4 to use this command!");
	for(new i;i<MAX_PLAYERS;i++)
	...
	format(string,sizeof(string),"* Administrator %s has Ubound all players *", AccountData[playerid][pAdminname]);
...
CMD:mute(playerid, params[]) {
	...
	if(sscanf(params, "u", otherid))
		return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/mute [name/playerid]");

	if(!IsPlayerConnected(otherid))
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Pemain tersebut tidak terkoneksi ke server!");

	if(SvMutePlayerStatus(otherid)) {
		SvMutePlayerDisable(otherid);
		SendClientMessageEx(otherid, -1, "[i] Anda telah di unmute oleh admin.");
	} else {
		SvMutePlayerEnable(otherid);
		SendClientMessageEx(otherid, -1, "[i] Anda telah di mute oleh admin.");
	}
	return 1;
}
...
CMD:ahealth(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_HEALTH, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay"WHITE" - Kesehatan", string, "Tutup", "");
    return 1;
}
...
CMD:checkdiscorducp(playerid, params[])
{
	...
	mysql_format(g_SQL, cQuery, sizeof(cQuery), "SELECT `ucp` FROM `playerucp` WHERE `DiscordID`='%s'", DCID);
	...
	{
		Error(playerid, "Tidak ditemukan akun user control panel dengan DiscordID: %s", DCID);
	}
...
CMD:aweap(playerid, params[])
{
	...
	for (new x = 1; x < MAX_WEAPON_SLOT; x ++)
	{
		...
		if(weaponid)
		{
			format(sha, sizeof(sha), "%s%s\t%d Ammo\n", sha, ReturnWeaponName(weaponid), ammo);
			found = true;
		}
	}
	...
	if(!found)
    {
        ...
        return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""NEXODUS"Croire Roleplay "WHITE"- Kepemilikan Senjata",
        "Pemain tersebut tidak memiliki senjata apapun!", "Tutup", "");
    }

	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST, sprintf(""NEXODUS"Croire Roleplay "WHITE"- %s", ReturnName(otherid)),
	sha, "Tutup", "");
	return 1;
}
...
CMD:setthestars(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" memberikan status "PINK1"The Stars"ARWIN1" kepada "YELLOW"%s(%d)"ARWIN1" selama 30 hari", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
...
CMD:osetstars(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah memberikan status the stars (offline) selama 30 Hari"ARWIN1" kepada account "YELLOW"%s", AccountData[playerid][pAdminname], playerName);

	new duration = gettime() + (30 * 86400); // 30 Hari
	mysql_format(g_SQL, cQuery, sizeof(cQuery), "UPDATE `player_characters` SET `Char_TheStars`=1, `Char_TheStarsTime`=%d WHERE `Char_Name`='%s'", duration, playerName);
	mysql_tquery(g_SQL, cQuery);
	return 1;
}
...
CMD:removestars(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah mencabut status "PINK1"The Stars"ARWIN1" pemain "YELLOW"%s(%d)", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
...
CMD:resetdelay(playerid, params[])
{
	...
	if(!strcmp(type, "mower", true))
	{
		...
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] Anda telah mereset delay sidejob mowing "YELLOW"%s(%d).", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob mowing anda.", GetAdminName(playerid));
	...
	else if(!strcmp(type, "delivery", true))
	{
		...
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob delivery "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob delivery anda.", GetAdminName(playerid));
	...
	else if(!strcmp(type, "sweeper", true))
	{
		...
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob sweeper "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob sweeper anda.", GetAdminName(playerid));
	...
	else if(!strcmp(type, "forklift", true))
	{
		...
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob forklift "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob forklift anda.", GetAdminName(playerid));
...
CMD:fightstyle(playerid, params[])
{
	...
	{
		Dialog_Show(playerid, DIALOG_SELECT_FSTYLE, DIALOG_STYLE_LIST, ""NEXODUS"Croire Roleplay "WHITE"- Fight Style",
		"Fight Normal\
		\n"GRAY"Fight Boxing\
		\nFight Kungfu\
		\n"GRAY"Fight KneeHead\
...
CMD:onlinelist(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay "WHITE"- List Online", string, "Tutup", "");
	return 1;
}
...
CMD:listafk(playerid, params[])
{
	...
	if(!find)
		Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""NEXODUS"Croire Roleplay "WHITE"- Player AFK", "Tidak ada pemain yang sedang AFK", "Tutup", "");
	else 
		Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay "WHITE"- Player AFK", string, "Tutup", "");
	return 1;		
}
...
CMD:customnumber(playerid, params[])
{
	...
	new query[598];
	mysql_format(g_SQL, query, sizeof(query), "SELECT Char_PhoneNum FROM player_characters WHERE Char_PhoneNum='%s'", number);
...
FUNC::RekeningCustom(playerid, otherid, rekening)
{
	...
	else
	{
		...
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah mengubah nomor rekening "YELLOW"%s(%d)"DARKORANGE" [%d] ~> [%d]", AccountData[playerid][pAdminname], ReturnName(otherid), otherid, oldrek, rekening);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mengubah nomor rekening anda. Sebelum "YELLOW"%d"ARWIN1" Sesudah "YELLOW"%d", AccountData[playerid][pAdminname], oldrek, rekening);
...
public PhoneNumberCustom(playerid, otherid, const number[])
{
	...
	else
	{
		...
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah customisasi nomor HP "YELLOW"%s(%d) - %s", GetAdminName(playerid), GetRPName(otherid), otherid, number);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] Admin "RED"%s"ARWIN1" telah mengganti nomor HP anda menjadi "YELLOW"%s.", GetAdminName(playerid), number);
		...
		new query[255];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE `player_characters` SET `Char_PhoneNum`='%s' WHERE `pID`=%d", number, AccountData[otherid][pID]);
		mysql_tquery(g_SQL, query);
	}
	return 1;
}
...
CMD:changegender(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah mengganti gender "YELLOW"%s(%d) - %s", GetAdminName(playerid), AccountData[id][pName], id, (gender == 1) ? "Laki-Laki" : "Perempuan");
	SendClientMessageEx(id, X11_GRAY, "[AdmCmd] Admin "RED"%s"ARWIN1" telah menetapkan gender anda menjadi %s.", GetAdminName(playerid), (gender == 1) ? "Laki-Laki" : "Perempuan");
...
CMD:respawnallveh(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah memulai waktu reloading player vehicle!");
	return 1;
}
...
CMD:torture(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menggunakan /torture kepada pemain "YELLOW"%s(%d)", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
	return 1;
}
...
CMD:giveredmoney(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" memberikan anda Red Money sebesar "RED"%s", AccountData[playerid][pAdminname], FormatMoney(value));
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" memberikan Red Money sebesar "RED"%s"ARWIN1" kepada pemain "YELLOW"%s(%d)", AccountData[playerid][pAdminname], FormatMoney(value), AccountData[otherid][pName], otherid);
...
{
	if(CheckAdmin(playerid, 6) && !IsPlayerAdmin(playerid))
		return PermissionError(playerid);

	if(g_ServerLocked)
		Dialog_Show(playerid, ServerPanel, DIALOG_STYLE_LIST, ""NEXODUS"Croire Roleplay "WHITE"- Server Panel", "Unlock Server\n"GRAY"Set Hostname\nSet Weburl\n"GRAY"Server Info", "Pilih", "Batal");

	else Dialog_Show(playerid, ServerPanel, DIALOG_STYLE_LIST, ""NEXODUS"Croire Roleplay "WHITE"- Server Panel", "Lock Server\n"GRAY"Set Hostname\nSet Weburl\n"GRAY"Server Info", "Pilih", "Batal");
	return 1;
}
		...
		switch(listitem)
		{
			case 0:// Lock/Unlock Server
			{
				if(g_ServerLocked)
				{
					...
					SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah membuka kunci Server", GetAdminName(playerid));
				}
				else Dialog_Show(playerid, LockServer, DIALOG_STYLE_INPUT, ""NEXODUS"Croire Roleplay "WHITE"- Lock Server", "Harap masukan password untuk mengunci server\n"YELLOW"(Masukan dikolom bawah ini):", "Lock", "Cancel");
			}
            case 1: Dialog_Show(playerid, SetHostname, DIALOG_STYLE_INPUT, ""NEXODUS"Croire Roleplay "WHITE"- Set Hostname", "Harap masukkan hostname baru server\n"YELLOW"(Masukan dikolom bawah ini):", "Submit", "Back");
            case 2: Dialog_Show(playerid, SetWeburl, DIALOG_STYLE_INPUT, ""NEXODUS"Croire Roleplay "WHITE"- Set Weburl", "Harap masukkan Weburl / MOTD Server yang akan digunakan\nCth: discord.gg/crrp", "Submit", "Batal");
			case 3: Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay "WHITE"- Server Information", "Category\tValue\nDynamic Object:\t%d\nVehicle's Count:\t%d", "Close", "", Streamer_CountItems(STREAMER_TYPE_OBJECT), GetVehiclePoolSize());
		}
	}
...
Dialog:LockServer(playerid, response, listitem, inputtext[])
{
    ...
    if(response)
    {
        if(isnull(inputtext) || !strcmp(inputtext, "0"))
            return Dialog_Show(playerid, LockServer, DIALOG_STYLE_INPUT, ""NEXODUS"Croire Roleplay "WHITE"- Lock Server", "Harap masukan password untuk mengunci server\n"YELLOW"(Masukan dikolom bawah ini):", "Lock", "Back");
        ...
        SendRconCommand(str);
        SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" has locked the server (password: %s).", GetAdminName(playerid), inputtext);
    }
...
Dialog:SetHostname(playerid, response, listitem, inputtext[])
{
    ...
    {
        if(isnull(inputtext))
            return Dialog_Show(playerid, SetHostname, DIALOG_STYLE_INPUT, ""NEXODUS"Croire Roleplay "WHITE"- Set Hostname", "Harap masukkan hostname baru server\n"YELLOW"(Masukan dikolom bawah ini):", "Submit", "Back");
        ...
        SendRconCommand(str);
        SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" has set the hostname to \"%s\".", GetAdminName(playerid), inputtext);
    }
    else callcmd::panel(playerid, "\1");
    return 1;
}
...
Dialog:SetWeburl(playerid, response, listitem, inputtext[])
{
	...
	if(isnull(inputtext)) return Dialog_Show(playerid, SetWeburl, DIALOG_STYLE_INPUT, ""NEXODUS"Croire Roleplay "WHITE"- Set Weburl",
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah mengubah MOTD Server menjadi \"%s\".", AccountData[playerid][pAdminname], inputtext);
	return 1;
}
...
CMD:restart(playerid, params[])
{
	...
	if(g_RestartServer)
	{
		...
		return SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] Waktu restart telah ditunda oleh "RED"%s.", GetAdminName(playerid));
	}
    ...
    SendClientMessageToAllEx(X11_GRAY, "[AdmCmd] Admin "RED"%s"ARWIN1" memulai waktu restart server "YELLOW"%d detik.", GetAdminName(playerid), times);
...
CMD:avehall(playerid, params[])
{
	...
	foreach(new i : Player)
	{
		...
		{
			TextDrawShowForPlayer(i, Textdraw_Asuransi[txd]);
		}
		PlayAudioStreamForPlayer(i, "https://b.top4top.io/m_3285x4nja1.mp3");
	}
...
CMD:respawnjobvehicle(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" Semua kendaraan job yang tidak dinaiki atau terpakai akan dihancurkan dalam 20 detik!", AccountData[playerid][pAdminname]);
...
Function: RespawnVehicleJobs(playerid)
{
	...
	SendClientMessageToAllEx(-1, ""ARWIN1"[AdmCmd] Semua kendaraan job yang tidak terpakai berhasil dihancurkan.");
	return 1;
}
...
{
	if(AccountData[playerid][pAdmin] < 1 && AccountData[playerid][pTheStars] < 1) return PermissionError(playerid);
	ShowPlayerDialog(playerid, DIALOG_ADMIN_HELP, DIALOG_STYLE_LIST, ""NEXODUS"Croire Roleplay "WHITE"- Admin Help",
	"Admin Magang\ 
	\n"GRAY"Helper\
	\nAdmin I\
	\n"GRAY"Junior Admin II\
	\nSenior Admin III\
	\n"GRAY"Management\
	\nFounder", "Pilih", "Batal");
	return 1;
...
CMD:dynhelp(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_DYNAMIC_HELP, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay "WHITE"- Dynamic Help", shstr, "Pilih", "Batal");
	return 1;
}
...
CMD:makequiz(playerid, params[])
{
	...
	if(!strcmp(type, "question", true))
	{
		...
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah membuat Quiz server.", AccountData[playerid][pAdminname]);
		SendClientMessageToAllEx(X11_YELLOW, "~> %s? - "GREEN"%s.", frmxt, FormatMoney(QuizPrice));
		SendClientMessageToAllEx(X11_YELLOW, "~> Gunakan "RED"'/qa'"ARWIN1" untuk menjawab quiz.");
		Quiz = true;
	}
	else if(!strcmp(type, "answer", true))
	{
		...
		if(Quiz) return ShowTDN(playerid, NOTIFICATION_ERROR, "Sudah ada Quiz sebelumnya akhiri dengan /makequiz end");
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] Anda telah menetapkan jawaban untuk Quiz ~> "YELLOW"%s", frmxt);
	...
	else if(!strcmp(type, "prize", true))
	{
		...
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] Anda telah menetapkan hadiah quiz sebesar "GREEN"%s.", FormatMoney(price));
		QuizPrice = price;
	}
	else if(!strcmp(type, "end", true))
	{
		...
		QuizPrice = 0;
		QuizAnswer = "";
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] Quiz telah diakhiri oleh "RED"%s.", GetAdminName(playerid));
	}
	return 1;
}
...
CMD:qa(playerid, params[])
{
	...
	if(!strcmp(frmtanswer, QuizAnswer, true)) 
	{
		...
		SendClientMessageToAllEx(X11_YELLOW, shstr);
...
CMD:stafflist(playerid, params[])
{
	...
	if(count == 0)
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""NEXODUS"Croire Roleplay "WHITE"- Staff List", "Tidak ada administrator yang online!", "Tutup", "");
	else
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay "WHITE"- Staff List", shstr, "Tutup", "");
	return 1;
}
...
CMD:checknetwork(playerid, params[])
{
	if (GetAdminLevel(playerid))
	{
		...
		Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""NEXODUS"Croire Roleplay "WHITE"- Network Stats", stats, "Close", "");
	}
	return 1;
}
...
CMD:admins(playerid, params[])
{
	...
	if(count == 0)
		Error(playerid, "Tidak ada administrator online dikota!"); // ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""NEXODUS"Nexodus Roleplay "WHITE"- Online Administrator", "Tidak ada administrator online dikota!", "Tutup", "");
	else
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay "WHITE"- Online Administrator", list, "Close", "");
	return 1;
}
...
CMD:ajail(playerid, params[])
{
	...
	if(count == 0)
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""NEXODUS"Croire Roleplay "WHITE"- Player Jail", "Tidak ada pemain dalam masa jail admin!", "Close", "");
	else 
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay "WHITE"- Player Jail", shstr, "Close", "");
	return 1;
}
...
CMD:aduty(playerid, params[])
{
	...
	if(!AccountData[playerid][pAdminDuty])
	{
		...
		{
			SendAdminMessage(X11_YELLOW, "[Staff Duty]"WHITE" %s %s telah on duty The Stars dengan nama %s.", GetStaffRank(playerid), AccountData[playerid][pName], GetAdminName(playerid));
		}
		else SendAdminMessage(X11_YELLOW, "[Staff Duty]"WHITE" %s %s telah on duty admin dengan nama %s.", GetStaffRank(playerid), AccountData[playerid][pName], GetAdminName(playerid));
	}
	else 
	{
		...
		if(AccountData[playerid][pTheStars])
		{
			SendAdminMessage(X11_YELLOW, "[Staff Duty]"WHITE" %s %s telah off duty The Stars dengan nama %s.", GetStaffRank(playerid), AccountData[playerid][pName], GetAdminName(playerid));
		}
		else SendAdminMessage(X11_YELLOW, "[Staff Duty]"WHITE" %s %s telah off duty admin dengan nama %s.", GetStaffRank(playerid), AccountData[playerid][pName], GetAdminName(playerid));
	}
	return 1;
}
...
CMD:togooc(playerid, params[])
{
	...
	if(!TogOOC)
	{
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mengaktifkan Global Chat OOC!", AccountData[playerid][pAdminname]);
		TogOOC = true;
	}
	else 
	{
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd]"RED"%s"ARWIN1" telah mematikan Global Chat OOC!", AccountData[playerid][pAdminname]);
		TogOOC = false;
	}
	return 1;
}
...
CMD:o(playerid, params[])
{
	...
	foreach(new i : Player) if (AccountData[i][pSpawned])
	{
		if(AccountData[i][pTogGlobal])
		{
			...
			{
				if(AccountData[playerid][pAdmin] > 0 && AccountData[playerid][pAdminDuty]) SendClientMessageEx(i, -1, "[G] %s "RED"%s: "WHITE"(( %s ))", GetStaffRank(playerid), AccountData[playerid][pAdminname], params);
				else if(AccountData[playerid][pVip] > 0 && !AccountData[playerid][pAdminDuty]) 
				{
					if(!strcmp(AccountData[playerid][pVipNameCustom], "-"))
					{
						SendClientMessageEx(i, -1, "[G] %s "WHITE"| %s [%d]: (( %s ))", GetPlayerLevelName(playerid), AccountData[playerid][pUCP], playerid, params);
					}
					else 
					{
						SendClientMessageEx(i, -1, "[G] %s "WHITE"| %s [%d]: (( %s ))", AccountData[playerid][pVipNameCustom], AccountData[playerid][pUCP], playerid, params);
					}
				}
				else SendClientMessageEx(i, -1, "[G] %s "WHITE"| %s [%d]: (( %s ))", GetPlayerLevelName(playerid), AccountData[playerid][pUCP], playerid, params);
			}
...
{	
	new otherid, hours, minutes, seconds;
	...
	SendClientMessageEx(playerid, -1, "[i] UCP: %s | Nama: %s | LV: %d | Ping: %d ms | Packet Loss: %.2f | Playtime: %02d:%02d:%02d | ID: %d", AccountData[otherid][pUCP], AccountData[otherid][pName], AccountData[otherid][pLevel], GetPlayerPing(otherid), GetPlayerPacketLoss(otherid), hours, minutes, seconds, otherid);
	return 1;
}
...
CMD:gotosf(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" teleportasi ke San Fiero", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:gotols(playerid, params[])
{
	...
	SetPlayerVirtualWorldEx(playerid, 0);
	SetPlayerInteriorEx(playerid, 0);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" Teleportasi ke Los Santos", GetAdminName(playerid));
	AccountData[playerid][pInDoor] = -1;
    AccountData[playerid][pInHouse] = -1;
    AccountData[playerid][pInBiz] = -1;
    AccountData[playerid][pInFamily] = -1;
...
CMD:gotoasur(playerid, params[])
{
	...
	SetPlayerVirtualWorldEx(playerid, 0);
	SetPlayerInteriorEx(playerid, 0);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" Teleportasi ke Insuranced", GetAdminName(playerid));
	AccountData[playerid][pInDoor] = -1;
    AccountData[playerid][pInHouse] = -1;
    AccountData[playerid][pInBiz] = -1;
    AccountData[playerid][pInFamily] = -1;
...
CMD:gotocarnaval(playerid, params[])
{
	...
	SetPlayerVirtualWorldEx(playerid, 0);
	SetPlayerInteriorEx(playerid, 0);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" Teleportasi ke Karnaval", GetAdminName(playerid));
	AccountData[playerid][pInDoor] = -1;
    AccountData[playerid][pInHouse] = -1;
    AccountData[playerid][pInBiz] = -1;
    AccountData[playerid][pInFamily] = -1;
...
CMD:goto(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" teleportasi ke player "YELLOW"%s(%d)", AccountData[playerid][pAdminname], ReturnName(otherid), otherid);
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd]:"RED"%s "ARWIN1"teleportasi ke posisi anda.", AccountData[playerid][pAdminname]);
	return 1;
}

alias:sendtoplayer("stp")
CMD:sendtoplayer(playerid, params[])
{
	new otherid, toother;
	...
	if(sscanf(params, "uu", otherid, toother)) return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/sendtoplayer [playerid/Name] to [playerid/name]");
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" mengirimkan anda kepada "YELLOW"%s(%d)", AccountData[playerid][pAdminname], AccountData[toother][pName], toother);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" Mengirimkan "YELLOW"%s(%d)"ARWIN1" kepada "YELLOW"%s(%d)", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid, AccountData[toother][pName], toother);
	return 1;
}
...
CMD:sendto(playerid, params[])
{
	...
	{
		Syntax(playerid, "/sendto [name/playerid] (ls, lv, sf, asuransi, carnaval, bandara, rs, balkot)");
		return 1;
	}
	...
	if(!strcmp(option, "ls", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Los Santos.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Los Santos.", AccountData[playerid][pAdminname]);
	...
	else if(!strcmp(option, "lv", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Las Vegas.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Las Vegas.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "sf", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke San Fiero.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke San Fiero.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "asuransi", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Asuransi.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Asuransi.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "carnaval", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Carnaval.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Carnaval.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "bandara", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Bandara.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Bandara.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "rs", true))
	{
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER) 
		{
            SetVehiclePos(GetPlayerVehicleID(otherid),2036.6349, -1409.5667, 17.2641);
        }
        else 
		{
            SetPlayerPositionEx(otherid, 2036.6349, -1409.5667, 17.2641, 184.0463, 2000);
        }
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Rumah Sakit.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Rumah Sakit.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "balkot", true))
	{
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER) 
		{
            SetVehiclePos(GetPlayerVehicleID(otherid),1246.8817, -2047.2511, 60.1056);
        }
        else 
		{
            SetPlayerPositionEx(otherid, 1246.8817, -2047.2511, 60.1056, 269.0607, 2000);
        }
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Balai Kota.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Balai Kota.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
...
CMD:gethere(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menarik pemain "YELLOW"%s(%d)"ARWIN1" ke posisinya.", AccountData[playerid][pAdminname], ReturnName(otherid), otherid);
	return 1;
}
...
CMD:areviveall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_GRAY, "[AdmCmd]"RED" %s"ARWIN1" telah menghidupkan semua player mati.", GetAdminName(playerid));
	return 1;
}
...
CMD:arevive(playerid, params[])
{
	...
	if(!isnull(params) && !strcmp(params, "me", true))
	{
		...
		return SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menghidupkan "YELLOW"diri sendiri", GetAdminName(playerid));
	}
	new 
		otherid
	;
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menghidupkan anda", GetAdminName(playerid));
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menghidupkan "YELLOW"%s(%d)", GetAdminName(playerid), AccountData[otherid][pName], otherid);
	return 1;
}
...
CMD:spec(playerid, params[])
{
	...
	if(!isnull(params) && !strcmp(params, "off", true))
	{
		...
		return SendClientMessageEx(playerid, X11_GRAY, "[AdmCmd] Kamu tidak lama dalam mode Spectating!");
	}
	new otherid;
	if(sscanf(params, "u", otherid))
    ...
    if(IsPlayerInAnyVehicle(otherid))
	{
		...
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER)
	    {
			SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] Anda sekarang Spectating"YELLOW" %s(%d)"ARWIN1" yang sedang mengemudikan "YELLOW"%s(%d)", AccountData[otherid][pName], otherid, GetVehicleModelName(GetVehicleModel(vID)), vID);
		}
		else
		{
			SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] Anda sekarang Spectating"YELLOW" %s(%d)"ARWIN1" yang merupakan penumpang di "YELLOW"%s(%d).", AccountData[otherid][pName], otherid, GetVehicleModelName(GetVehicleModel(vID)), vID);
		}
	}
    else
	{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" sedang memantau "YELLOW"%s(%d).", GetAdminName(playerid), GetRPName(otherid), otherid);
	return 1;
}
...
CMD:slap(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah melakukan slaped ke pemain "YELLOW"%s(%d).", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
	PlayerPlaySound(otherid, 1130, 0.0, 0.0, 0.0);
	return 1;
}
CMD:count(playerid, params[])
{
    if (AccountData[playerid][pAdmin] < 1 && !AccountData[playerid][pTheStars])
        return PermissionError(playerid);

    SendClientMessageEx(playerid, X11_YELLOW, "(Server) "WHITE"This server has "YELLOW"%d total players.", Iter_Count(Player));
    SendClientMessageEx(playerid, X11_YELLOW, "(Server) "WHITE"This server has "YELLOW"%d total vehicles.", Iter_Count(Vehicle));
    return 1;
}
...
CMD:jetpack(playerid, params[])
{
	...
	if(sscanf(params, "u", otherid))
	{
		...
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" Menggunakan Jetpack", AccountData[playerid][pAdminname]);
	}
	else 
	{
		AccountData[otherid][pJetpack] = true;
		SetPlayerSpecialAction(otherid, SPECIAL_ACTION_USEJETPACK);
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" memberikan jetpack kepada "YELLOW"%s(%d).", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
	}
	return 1;
}
...
CMD:getip(playerid, params[]) 
{
	...
	if(AccountData[otherid][pAdmin] > 5)
	{
		...
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s(%d)"ARWIN1" mencoba mengambil data ip anda.", AccountData[playerid][pAdminname], playerid);
		return 1;
	}
	GetPlayerName(otherid, givePlayer, sizeof(givePlayer));
	GetPlayerIp(otherid, PlayerIP, sizeof(PlayerIP));
	
	SendClientMessageEx(playerid, X11_YELLOW, "[AdmCmd] Details IP Pemain "YELLOW"%s(%d) ~> "GREEN"`%s`.", givePlayer, otherid, PlayerIP);
	return 1;
}
...
CMD:aka(playerid, params[])
{
	...
	GetPlayerIp(otherid, PlayerIP, sizeof(PlayerIP));
	mysql_format(g_SQL, query, sizeof(query), "SELECT Char_Name FROM player_characters WHERE Char_IP='%s'", PlayerIP);
...
CMD:akaip(playerid, params[])
{
	...
	mysql_format(g_SQL, query, sizeof(query), "SELECT Char_Name FROM player_characters WHERE IP='%s'", params);
...
public OfflineWarnPlayer(playerid, name[], reason[])
{
	...
	else 
	{
		...
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] Akun "YELLOW"%s"ARWIN1" telah diberikan warning(offline) oleh "RED"%s.", name, AccountData[playerid][pAdminname]);
		SendClientMessageToAllEx(X11_YELLOW, "~> Alasan: %s.", reason);

		new query[255];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE `player_characters` SET `Char_Warn`=%d WHERE `pID`=%d", Warn + 1, RegID);
		mysql_tquery(g_SQL, query);

		mysql_format(g_SQL, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=1, `WarnSender`='%s', `WarnTime`=%d, `WarnReason`='%s'", RegID, AccountData[playerid][pAdminname], gettime(), reason);
		mysql_tquery(g_SQL, query);
	}
	return 1;
}
...
public OfflineJailed(adminid, const NameJail[], const Reason[], time)
{
	...
	else 
	{
		...
		cache_get_value_index_int(0, 0, RegID);
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] Pemain dengan nama "YELLOW"%s"ARWIN1" telah dijail (offline) selama "RED"%d menit "ARWIN1"oleh"RED" %s", NameJail, time, GetAdminName(adminid));
		SendClientMessageToAllEx(X11_YELLOW, "~> Alasan: %s", Reason);

		new query[522], tmp[200], date;
		date = gettime();
		mysql_format(g_SQL, query, sizeof(query), "UPDATE `player_characters` SET `Char_Jail`=1, `Char_JailTime`=%d, `Char_JailReason`='%s' WHERE `pID`=%d", Jailtime, Reason, RegID);
		mysql_tquery(g_SQL, query);

		mysql_format(g_SQL, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=2, `WarnTime`=%d, `WarnSender`='%s', `WarnReason`='%s'", RegID, date, GetAdminName(adminid), Reason);
		mysql_tquery(g_SQL, query);
...
CMD:jail(playerid, params[])
{
	...
	date = gettime();
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=2, `WarnTime`=%d, `WarnSender`='%s', `WarnReason`='%s'", AccountData[otherid][pID], date, GetAdminName(playerid), reason);
	mysql_tquery(g_SQL, query);
...
CMD:aunarrest(playerid, params[])
{
	...
	AccountData[otherid][pArrest] = 0;
	AccountData[otherid][pArrestTime] = 0;
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" Membebaskan anda dari Arrest (Penjara Polisi)", AccountData[playerid][pAdminname]);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" Membebaskan player "YELLOW"%s(%d)"ARWIN1" dari Arrest (Penjara Polisi)", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
...
CMD:unjail(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah membebaskan anda dari penjara!", GetAdminName(playerid));
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah membebaskan "YELLOW"%s(%d)"ARWIN1" dari hukuman jail!", GetAdminName(playerid), GetRPName(otherid), otherid);
...
CMD:kick(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd]"YELLOW" %s(%d)"ARWIN1" telah ditendang dari server oleh "RED"%s.", AccountData[otherid][pName], otherid, AccountData[playerid][pAdminname]);
	SendClientMessageToAllEx(X11_YELLOW, "~> Alasan: %s", reason);
	KickEx(otherid);
	return 1;
}
...
CMD:blockucp(playerid, params[])
{
	...
	if(AccountData[playerid][pAdmin] < 3) return PermissionError(playerid);
	if(sscanf(params, "s[24]ds[128]", username, datez, reason)) return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/blockucp [ucp name] [waktu [hari]] [reason]");
	...
	if(Blacklist_Exists("name", username))
		return ShowTDN(playerid, NOTIFICATION_ERROR, "UCP telah di banned sebelumnya!");
	
	foreach(new i : Player) if (!strcmp(AccountData[i][pUCP], username))
	{
		if(AccountData[i][pAdmin] > AccountData[playerid][pAdmin]) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak dapat memblokir Akun UCP admin diatas anda!");
	}
	
	if(datez != 0)
	{
		banTime = gettime() + (datez * 86400);
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah memblokir Akun UCP "YELLOW"%s"ARWIN1" dari server selama %d hari.", AccountData[playerid][pAdminname], username, datez);
		SendClientMessageToAllEx(X11_YELLOW, "Reason ~> %s", reason);
	}
	else
	{
		banTime = datez;
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah memblokir "RED"Permanent"ARWIN1" Akun UCP "YELLOW"%s"ARWIN1" dari server.", AccountData[playerid][pAdminname], username, datez);
		SendClientMessageToAllEx(X11_YELLOW, "Reason ~> %s", reason);
	}

	new query[248];
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO `player_bans` (`name`, `ip`, `admin`, `reason`, `ban_date`, `ban_expire`) VALUES ('%s', ' ', '%s', '%s', '%d', '%d')", username, AccountData[playerid][pAdminname], reason, gettime(), banTime);
	mysql_tquery(g_SQL, query);
	return 1;
}
...
CMD:ban(playerid, params[])
{
	...
	if(datez != 0)
	{
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah membanned akun player"YELLOW" %s(%d)"ARWIN1" selama "YELLOW"%d"ARWIN1" hari", GetAdminName(playerid), playerName, otherid, datez);
		SendClientMessageToAllEx(X11_YELLOW, "~> Alasan: %s", reason);
	...
	{
		SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah membanned "RED"Permanent"ARWIN1" player"YELLOW" %s(%d)", GetAdminName(playerid), playerName, otherid, datez);
		SendClientMessageToAllEx(X11_YELLOW, "~> Alasan: %s", reason);
    ...
    {
		banTime = gettime() + (datez * 86400);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"Temporary Banned"ARWIN1" kepada anda selama %d hari.", datez);
	}
	else
	{
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd]"RED" Permanent Banned"ARWIN1" harap kontak admin yang bersangkutan jika anda merasa tidak bersalah!");
		banTime = datez;
	}
	new query[248], date;
	date = gettime();
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO `player_bans` (`name`, `ip`, `admin`, `reason`, `ban_date`, `ban_expire`) VALUES ('%s', '%s', '%s', '%s', '%d', '%d')", AccountData[otherid][pName], PlayerIP, AccountData[playerid][pAdminname], reason, gettime(), banTime);
	mysql_tquery(g_SQL, query);

	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnTime`=%d, `WarnType`=3, `WarnSender`='%s', `WarnReason`='%s'", AccountData[otherid][pID], date, AccountData[playerid][pAdminname], reason);
	mysql_tquery(g_SQL, query);
...
CMD:unban(playerid, params[])
{
	...
	new query[128];
	mysql_format(g_SQL, query, sizeof(query), "SELECT name,ip FROM player_bans WHERE name = '%s'", tmp);
...
public OnUnbanQueryData(adminid, BannedName[])
{
	if(cache_num_rows() > 0)
	{
		...
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah mencabut status banned dari akun "YELLOW"%s", AccountData[adminid][pAdminname], BannedName);
...
function UnbanUCP(adminid, BannedName[])
{
	if(cache_num_rows() > 0)
	{
		...
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah melepas hukuman banned "YELLOW"%s"ARWIN1" dari server", GetAdminName(adminid), BannedName);
	}
	else
	{
...
CMD:awarn(playerid, params[])
{
	...
	if(cache_num_rows())
	{
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""NEXODUS"Croire Roleplay "WHITE"- %s(%d)", AccountData[otherid][pName], otherid), list, "Tutup", "");
	}
	else
	{
		new list[255];
...
CMD:warn(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah diberikan warning oleh "RED"%s.", AccountData[otherid][pName], otherid, GetAdminName(playerid));
	SendClientMessageToAllEx(X11_YELLOW, "~> Alasan: %s", reason);

	new query[255], currentTime;
	currentTime = gettime();
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=1, `WarnSender`='%s', `WarnTime`=%d, `WarnReason`='%s'", AccountData[otherid][pID], GetAdminName(playerid), currentTime, reason);
	mysql_tquery(g_SQL, query);
	return 1;
}
...
CMD:unwarn(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menghapus 1 point warning anda!", GetAdminName(playerid));
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menghapus 1 point warning dari %s", GetAdminName(playerid), AccountData[otherid][pName]);
	return 1;
}
        ...
        if(GetVehicleModel(i))
        {
            ...
            if (((tempposx < radi) && (tempposx > -radi)) && ((tempposy < radi) && (tempposy > -radi)) && ((tempposz < radi) && (tempposz > -radi)))
            {
				if(IsVehicleEmpty(i))
				{
					...
					SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah merespawn kendaraan disekitarnya dengan radius"YELLOW" %.1f", GetAdminName(playerid), radi);
				}
			}
        }
    }
...
CMD:respawnsidejob(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah memulai timer selama 10 detik untuk merespawn kendaraan sidejob yang tidak terpakai / dinaiki.", AccountData[playerid][pAdminname]);
	return 1;
}
...
public RespawnPVSideJob(playerid)
{
	...
	SendClientMessageToAll(X11_YELLOW, "[AdmCmd] Semua kendaraan sidejob yang tidak terpakai/dinaiki telah direspawn kembali");
	return 1;
}
...
CMD:sethp(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan health anda menjadi %d", GetAdminName(playerid), value);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menetapkan health "YELLOW"%s(%d) - %d%", GetAdminName(playerid), GetRPName(otherid), otherid, value);
	return 1;
}
...
CMD:sethpall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan health bagi seluruh pemain menjadi %d", GetAdminName(playerid), value);
	return 1;
}
...
CMD:setbone(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah men set jumlah Kondisi tulang pemain "YELLOW"%s(%d).", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
	SendClientMessageEx(otherid, -1, ""ARWIN1"Admin "RED"%s"ARWIN1" telah men set Kondisi tulang anda", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:setboneall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah men set jumlah Kondisi tulang seluruh pemain online menjadi %d%%.", AccountData[playerid][pAdminname], jumlah);
	return 1;
}
...
CMD:setam(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menetapkan armour "YELLOW"%s(%d) - %d.00", GetAdminName(playerid), GetRPName(otherid), otherid, jumlah);
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan armour anda menjadi %d.00", GetAdminName(playerid), jumlah);
	return 1;
}
...
CMD:vsethealth(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" menetapkan "YELLOW"%.2f"ARWIN1" Health untuk kendaraan"YELLOW" [VID: %d] [%s]", AccountData[playerid][pAdminname], health, vehid, GetVehicleName(vehid));
	return 1;
}
...
CMD:afix(playerid, params[])
{
	...
	if(vehicleid > 0 && isnull(params))
	{
		...
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" memperbaiki kendaraan "YELLOW"[VID: %d] [%s]", AccountData[playerid][pAdminname], vehicleid, GetVehicleName(vehicleid));
	}
	else
	{
...
CMD:setskin(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" menetapkan skin"YELLOW" %s(%d) - %d.", GetAdminName(playerid), AccountData[otherid][pName], otherid, skin);
	return 1;
}
...
CMD:settime(playerid, params[])
{
	...
	new time;
	if(sscanf(params, "d", time)) ShowTDN(playerid, NOTIFICATION_SYNTAX, "/time [time]"); 
...
//	WorldTime = time;
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah mengubah waktu in-game server", GetAdminName(playerid));
	return 1;
}
...
CMD:setweather(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah mengubah cuaca in-game server.", AccountData[playerid][pAdminname]);
    return 1;
}
...
CMD:resetweap(playerid, params[])
{
    ...
    SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] Admin "RED"%s"ARWIN1" telah mereset senjata anda.", AccountData[playerid][pAdminname]);
    SendStaffMessage(X11_YELLOW, ""RED"%s "ARWIN1"telah mereset senjata "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
	return 1;
}
...
CMD:setlevel(playerid, params[])
{
	...
	if(AccountData[otherid][newCitizen]) {
		if(jumlah <= 1) return ShowTDN(playerid, NOTIFICATION_ERROR, "Level yang di set harus lebih dari 1.");
		AccountData[otherid][newCitizen] = false;
		AccountData[otherid][newCitizenTimer] = 0;
		forex(i, 10) PlayerTextDrawHide(playerid, UI_NEWPLAYER[playerid][i]);
	}
	AccountData[otherid][pLevel] = jumlah;
	SetPlayerScore(otherid, jumlah);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menetapkan level kepada "YELLOW"%s(%d) - %d", GetAdminName(playerid), GetRPName(otherid), otherid, jumlah);
	return 1;
}
...
CMD:setstress(playerid, params[])
{
	...
	SendClientMessageEx(userid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menetapkan stress anda menjadi %d%%", AccountData[playerid][pAdminname], jumlah);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menetapkan stress "YELLOW"%s(%d)"ARWIN1" menjadi %d%%", AccountData[playerid][pAdminname], AccountData[userid][pName], userid, jumlah);
	return 1;
}
...
CMD:reco(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s "ARWIN1" telah mereset ekonomi "YELLOW"%s(%d)", GetAdminName(playerid), GetRPName(userid), userid);
	SendClientMessageEx(userid, X11_GRAY, "[i] Admin "RED"%s"ARWIN1" telah mereset ekonomi anda", GetAdminName(playerid));
...
CMD:setstressall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_GRAY, "[AdmCmd]"RED" %s "ARWIN1"telah menetapkan stress seluruh pemain menjadi %d", GetAdminName(playerid), value);
	return 1;
}
...
CMD:goodmood(playerid, params[])
{
	...
	if(!AccountData[playerid][pGoodMood])
	{
		...
		SendStaffMessage(X11_YELLOW, "%s"ARWIN1" menggunakan cmd '/goodmood'", GetAdminName(playerid));
	}
	else
	{
		AccountData[playerid][pGoodMood] = false;
		SendStaffMessage(X11_YELLOW, "%s"ARWIN1" berhenti dari mode '/goodmood'", GetAdminName(playerid));
	}
	return 1;
}
...
CMD:sethbe(playerid, params[])
{
	...
	SendClientMessageEx(targetid, X11_GRAY, "[i] Admin"RED" %s"ARWIN1" telah menetapkan HBE anda menjadi %d", GetAdminName(playerid), value);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menetapkan HBE kepada "YELLOW"%s(%d) - %d", GetAdminName(playerid), GetRPName(targetid), targetid, value);
	return 1;
}
...
CMD:sethbeall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan HBE untuk seluruh player menjadi %d", GetAdminName(playerid), value);
	return 1;
}
...
CMD:setvip(playerid, params[])
{
	...
	if(dayz == 0)
	{
		...
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menetapkan VIP %s"ARWIN1" kepada "YELLOW"%s(%d)."ARWIN1" Durasi: Permanent.", GetAdminName(playerid), GetPlayerVipName(otherid), GetRPName(otherid), otherid);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] Admin "RED"%s "ARWIN1"telah menetapkan VIP Kepada anda ke level %s"ARWIN1" Permanent time!", GetAdminName(playerid), GetPlayerVipName(otherid));
	}
	else
	{
		AccountData[otherid][pVipTime] = gettime() + (dayz * 86400);
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menetapkan VIP %s"ARWIN1" kepada "YELLOW"%s(%d)."ARWIN1" Durasi: %d hari.", GetAdminName(playerid), GetPlayerVipName(otherid), GetRPName(otherid), otherid, dayz);
		SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] Admin "RED"%s "ARWIN1"telah menetapkan VIP Kepada anda ke level %s"ARWIN1" selama %d hari!", GetAdminName(playerid), GetPlayerVipName(otherid), dayz);
	}
...
CMD:giveweapall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah memberikan "YELLOW"%s(%d ammo)"ARWIN1" kepada seluruh pemain lebih dari level %d", AccountData[playerid][pAdminname], ReturnWeaponName(weaponid), ammo, level);
...
CMD:giveweap(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s "ARWIN1"memberikan "YELLOW"%s(%d Ammo)"ARWIN1" kepada "YELLOW"%s(%d).", GetAdminName(playerid), ReturnWeaponName(weaponid), ammo, AccountData[otherid][pName], otherid);
...
CMD:takemoney(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] Admin "RED"%s"ARWIN1" telah menarik uang anda sejumlah "DARKGREEN"%s.", GetAdminName(playerid), FormatMoney(value));
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menarik uang milik "YELLOW"%s(%d) -> %s.", GetAdminName(playerid), AccountData[otherid][pName], otherid, FormatMoney(value));
...
CMD:veh(playerid, params[])
{
    ...
    SetVehicleNumberPlate(vehicleid, ""RED"ADMIN");
	SendStaffMessage(X11_YELLOW, ""RED"%s "ARWIN1"memunculkan kendaraan %s dengan warna (%d, %d).", GetAdminName(playerid), GetVehicleModelName(model[0]), color1, color2);
	return 1;
}
...
{
	if (!AccountData[playerid][IsLoggedIn]) return Info(playerid, "Kamu belum login.");
 	new
		date[6];
	...
	SendClientMessageEx(playerid, X11_YELLOW, "The date is: %d/%d/%d -- The time is: %02d:%02d:%02d", date[2], date[1], date[0], date[3], date[4], date[5]);
	return true;
}
CMD:boost(playerid, params[])
{
    ...
    if (sscanf(params, "f", speed))
        return Info(playerid, "/boost [speed]");

    if (!IsPlayerInAnyVehicle(playerid))
        return Info(playerid, "You must in any vehicle to use this command!");

    if (GetPlayerState(playerid) != PLAYER_STATE_DRIVER)
        return Info(playerid, "You must to be driver to use this command!");
...
CMD:ledak(playerid, params[])
{
    ...
    new userid;
    if (sscanf(params, "u", userid))
        return Info(playerid, "/ledak [playerid/name]");

    if (!IsPlayerConnected(userid))
        return Info(playerid, "That player isn't connected and not spawned");
		...
		if(vehid == PlayerVehicle[i][pVehPhysic])
		{
			foreach(new pid : Player)
			{
				if(PlayerVehicle[i][pVehOwnerID] == AccountData[pid][pID]) {
					...
					SendClientMessageEx(pid, X11_GRAY, "[AdmCmd] Admin "RED"%s"ARWIN1" telah mengcustom plate kendaraan anda dengan nomor plate: "YELLOW"%s", GetAdminName(playerid), string);
					SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" menggunakan cmd '/customplate' pada kendaraan milik"YELLOW"%s(%d) - Plat %s.", GetAdminName(playerid), AccountData[pid][pName], pid, string);
				}
			}
		}
	}
...
CMD:adveh(playerid, params[])
{
	...
	if(GetPlayerState(playerid) == PLAYER_STATE_ONFOOT) //jika player sedang di tanah
	{
		...
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menghancurkan semua kendaraan static tidak terpakai milik Admin.", GetAdminName(playerid));
	}
	else //jika player tidak di tanah atau sedang didalam kendaraan
	{
		...
		SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" menghancurkan kendaraan static admin VID: %d.", GetAdminName(playerid), vehicleid);
	}
	return 1;
}
//-----------------------------[ Admin Level 5 ]------------------
...
CMD:setadminname(playerid, params[])
{
	...
	new query[125];
	mysql_format(g_SQL, query, sizeof(query), "SELECT `Char_AdminName` FROM `player_characters` WHERE `Char_AdminName`='%s'", aname);
...
CMD:setmoney(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan uang anda menjadi "LIGHTGREEN"%s", GetAdminName(playerid), FormatMoney(money));
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah menetapkan uang "YELLOW"%s(%d) - %s", AccountData[playerid][pAdminname], GetRPName(otherid), otherid, FormatMoney(money));
...
CMD:givemoney(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] Admin "RED"%s"ARWIN1" memberikan anda uang sejumlah "DARKGREEN"%s.", GetAdminName(playerid), FormatMoney(value));
	SendStaffMessage(X11_YELLOW, ""RED"%s "ARWIN1"memberikan uang kepada "YELLOW"%s(%d) - %s", GetAdminName(playerid), AccountData[otherid][pName], otherid, FormatMoney(value));
...
CMD:givemoneyall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_YELLOW, "[AdmCmd]"RED" %s"ARWIN1" telah memberikan uang kepada player online dengan jumlah "DARKGREEN"%s.", GetAdminName(playerid), FormatMoney(value));
...
CMD:setbankmoney(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] Admin "RED"%s"ARWIN1" menetapkan Saldo Rekening anda menjadi "DARKGREEN"%s.", GetAdminName(playerid), FormatMoney(value));
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" menetapkan Saldo Rekening "YELLOW"%s(%d) - %s.", GetAdminName(playerid), AccountData[otherid][pName], otherid, FormatMoney(value));
...
CMD:setvw(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" menetapkan WWID "YELLOW"%s(%d) - %d", GetAdminName(playerid), AccountData[otherid][pName], otherid, wwid);
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah men set VWID anda - %d.", AccountData[playerid][pAdminname], wwid);
	return 1;
}
...
CMD:setint(playerid, params[])
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" menetapkan INT ID "YELLOW"%s(%d) - %d", GetAdminName(playerid), AccountData[otherid][pName], otherid, intid);
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah men set Int ID anda - %d.", AccountData[playerid][pAdminname], intid);
	return 1;
}
...
CMD:explode(playerid, params[])
{
	...
	GetPlayerPos(otherid, POS[0], POS[1], POS[2]);
	CreateExplosion(POS[0], POS[1], POS[2], 7, 5.0);
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" menggunakan explode kepada "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
...
CMD:setadmin(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_YELLOW, "[i] Admin "RED"%s"ARWIN1" telah menetapkan anda sebagai %s", GetAdminName(playerid), GetStaffRank(otherid));
	SendStaffMessage(X11_YELLOW, ""RED"%s "ARWIN1"telah menetapkan admin kepada "YELLOW"%s(%d) - %s.", GetAdminName(playerid), AccountData[otherid][pName], otherid, GetStaffRank(otherid));
...
CMD:kickallplayer(playerid, params[])
{
	...
	foreach(new pid : Player) {
		if(AccountData[pid][pAdmin] < 1) {
			...
			SendClientMessageEx(pid, -1, "[i] Maaf server telah Badai, semua Database Telah tersimpan di pusat "YELLOW"Verona");
			KickEx(pid);
		}
	}
	return 1;
...
{
	if(AccountData[playerid][pAdmin] < 1) return PermissionError(playerid);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, ""NEXODUS"Croire Roleplay "WHITE"- Job List",
	"0\tPengangguran\
	\n"GRAY"1\tSupir Bus\
	\n2\tPenambang\
	\n"GRAY"3\tTukang Kayu\
...
CMD:setjob(playerid, params[])
{
	...
	if(jobid < 0 || jobid > 13) return Error(playerid, "Invalid Job ID!");
	
	AccountData[otherid][pJob] = jobid;
	LoadPlayerJobIcon(otherid);

	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" telah set job "YELLOW"%s(%d) - %s", GetAdminName(playerid), AccountData[otherid][pName], otherid, GetPlayerJobName(otherid));
	SendClientMessageEx(otherid, X11_YELLOW, "[AdmCmd] "RED"%s"ARWIN1" telah menetapkan pekerjaan"YELLOW" %s "ARWIN1"untuk anda", GetAdminName(playerid), GetPlayerJobName(otherid));
	return 1;
}
...
CMD:clearallchat(playerid)
{
	...
	SendStaffMessage(X11_YELLOW, ""RED"%s"ARWIN1" menghapus semua chatlog", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:fixes(playerid, params[])
{
	...
	if(count == 0)
	{
		...
		return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""NEXODUS"Croire Roleplay "WHITE"- Fix Request",
		"Tidak ada player request fix untuk saat ini!", "Tutup", "");
	}

	ShowPlayerDialog(playerid, DIALOG_FIXMEACC, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay"WHITE" - Fix Request", list, "Pilih", "Batal");
	return 1;
}
...
public OfflineStats(playerid, const name[])
{
	...
	{
		new RegID, UCPName[42], PName[64], Age[64], Height, Weight, Origin[64], Gender, Job, Phone[64],
		...
		cache_get_value_index(0, 9, Phone);
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""NEXODUS"Croire Roleplay "WHITE"- Offline Stats", shstr, "Tutup", "");
